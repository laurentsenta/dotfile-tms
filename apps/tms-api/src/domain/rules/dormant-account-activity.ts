...
/**
 * Rule ID for dormant account activity detection
...
/**
 * Configuration for the dormant account activity rule
 */
const CONFIG = {
  // Number of days of inactivity to consider an account dormant
  DORMANCY_THRESHOLD_DAYS: 90,
  
  // Number of transactions in a day to consider suspicious for a previously dormant account
  SUSPICIOUS_DAILY_TX_THRESHOLD: 5,
  
  // TTL for the dormant flag in seconds (24 hours)
 ...
 * @param history The account history service to check for dormancy and daily transactions
...
export async function dormantAccountActivity(
  ...
  const isDormant = !previousActivityDate || daysSinceLastActivity >= CONFIG.DORMANCY_THRESHOLD_DAYS;
  
  if (isDormant) {
    // Set the dormant flag with a TTL
    await history.setWasDormant(transaction.sourceAccountKey, CONFIG.DORMANT_FLAG_TTL);
  } else {
    // If not dormant, check if it was previously flagged as dormant
    const wasDormant = await history.getWasDormant(transaction.sourceAccountKey);
    
    if (wasDormant) {
      // Get the daily transaction count for this account
      const dailyTxTotal = await history.getDailyTxTotal(
        transaction.sourceAccountKey,
        day
      );
      
      // Check if the daily transaction count exceeds the threshold
